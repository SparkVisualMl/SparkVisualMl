...
public class SwaggerConfig {
    ...
    public Docket createRestApi(){
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("model API 接口使用文档").apiInfo(apiInfo()) //组名
                .select() //选择哪些路径和api会生成doucument
                //为当前包下的api路径监控
                .apis(RequestHandlerSelectors.basePackage("com.spark.control"))
    ...
    private  ApiInfo apiInfo(){
        return new ApiInfoBuilder()
                .title("Spark可视化机器学习接口使用文档")//文档页面标题
                .contact(new Contact("lilingui","http://sparkvisualml.com","javalovelinux@163.com"))//创建人
                .version("1.0")  //版本号
                .description("Api 接口使用指南")
                .build();
    }
